...
if __name__ == "__main__":
    ...
    mean_songs_playlist = (
        df.groupby(["playlist", "commit_date"])
        .agg({"song_id": "nunique"})
        .reset_index()
        .groupby("playlist")
        .agg({"song_id": "mean"})
    ...
    non_changing_playlists = (
        ...
        .agg({"song_id": "nunique"})
        .merge(
            mean_songs_playlist.rename({"song_id": "number_of_songs"}, axis="columns"),
            how="left",
            left_index=True,
            right_index=True,
        )
        .assign(same_songs=lambda x: x["song_id"] / x["number_of_songs"])
    ...
    songs = (
        df.query(
            "commit_date == @commit_date and playlist not in @non_changing_playlists and date > @last_week.tz_localize(None)"
        )
        .explode("artists")
        ...
        .agg(
            {
                ...
                "cover_url": "unique",
                "playlist": "unique",
                "seen in": lambda x: ", ".join(x.explode().unique().tolist()),
            }
        )
        .query(
            '~artists.str.contains("Double P|Peso Pluma|Carín León", case=False, na=False)'
        )
